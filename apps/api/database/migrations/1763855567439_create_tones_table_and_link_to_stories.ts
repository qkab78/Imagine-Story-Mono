...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db
    ...
    .values([
      {
        name: 'happy',
        description: 'Tonalité joyeuse et positive, parfaite pour des histoires réconfortantes et optimistes'
      },
      {
        name: 'calm',
        description: 'Tonalité apaisante et douce, idéale pour des histoires relaxantes avant le coucher'
      },
      {
        name: 'adventurous',
        description: 'Tonalité aventureuse et excitante, pour des histoires d\'exploration et de découverte'
      },
      {
        name: 'magical',
        description: 'Tonalité magique et merveilleuse, pour des contes fantastiques et imaginaires'
      },
      {
        name: 'educational',
  ...
  await db.schema
    .alterTable('stories')
    .addColumn('tone_id', 'uuid', (col) => col
      .references('tones.id')
      .onDelete('set null')
    )
    .execute()
